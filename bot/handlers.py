...
async def period_selected(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.answer()  # Подтверждаем callback, чтобы убрать крутилки в интерфейсе

@router.callback_query(lambda c: c.data.startswith("category_"))
async def category_selected(callback: types.CallbackQuery, state: FSMContext):
    await callback.answer("Начинаю обработку...")  # Подтверждаем callback
    ...
    # Сопоставляем выбранный период с числом дней
    period_to_days = {
        'day': 1,
        'week': 7,
        'month': 30
    ...
    try:
        ...
        all_news = await fetch_news_from_channels(period_days=days)  # Передаем правильный параметр
